...
export default function ActivityPathCarousel({ images, setSelectedImage }: Props) {
  ...
  if (!isMounted) {
    return <div ref={containerRef} className="relative w-full h-[200px] bg-gray-100 rounded-lg flex items-center justify-center">
  ...
  return (
    ...
    >
      ...
      {/* Thumbnails */}
      {pathLength > 0 && images.map((image, index) => {
        ...
        return (
          <motion.button
            ...
            className={`absolute rounded-full overflow-hidden border-2 shadow-lg ${
              isActive ? "border-blue-500 z-20 shadow-xl" : "border-gray-300 z-10"
            }`}
            style={{
